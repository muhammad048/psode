...
def evolve(score_func, de_iter, population, score_lst, paras1, paras2, model, loader_de, args):
    evolve_out = None
    if args.local_rank == 0:
        pop_new, params_for_update = mutate_and_crossover(de_iter, population, score_lst, paras1, paras2, args) #, *_ 
    ...
    print(de_iter,"new_population_f",new_population_f)
    if args.local_rank == 0:
        evolve_out = update_pop(de_iter, pop_new, population, new_population_f, score_lst, params_for_update, args)

    if args.distributed: dist.barrier()
    return evolve_out
    # population, population_f, bestidx, worstidx, dist_matrics, paras2, update_label

def mutate_and_crossover(iter, population, population_f, paras1, paras2, args):
    ...
    for idx in range(0, popsize):
        ...
        id1 = np.random.choice(list(set(range(0, popsize)) - {idx}), 1, replace=False)[0]
        if k_c<4:
            if iter < max_iters/2:
                ...
                # With p1, we determine which strategy to use
                if p1 > np.random.rand():
                    f = f_init * (np.sin(2*np.pi*freq*iter + np.pi)*(max_iters-iter)/max_iters + 1)
                    ...
                    f = f_init * (np.sin(2*np.pi*freq_i*iter)*iter/max_iters + 1)
        ...
        # bestidx = population_f.index(max(population_f))
        bestidx = find_max_index_after_removing_idx(population_f, idx)
        # print('id1,idx................', id1, idx)
        if k_c==0:
            ...
            if popsize <= 20:
                p_total = population_f[idx] + population_f[id1] + population_f[bestidx]
                p1_, p2_, p3_ =  population_f[idx]/p_total, population_f[id1]/p_total, population_f[bestidx]/p_total
                x_new = population[idx]*(1./3 + p2_ + p3_ - 2*p1_) + population[id1]*(1./3 + p1_ + p3_ - 2*p2_) +\
                        population[bestidx]*(1./3 + p1_ + p2_ - 2*p3_)
                pos_new = x_new
...
def update_pop(iter, pop_new, population, new_population_f, population_f, params_for_update, args, **kwargs):
    ...
    for idx in range(0, popsize): 
        ...
        update_summary(iter, parameter3, os.path.join(args.output_dir, 'summary_strategy.csv'), write_header=True) 
    ...
    # number of trials in recent 10 iter
    recent_size = 10*popsize
    ...
    update_summary(iter, parameter3, os.path.join(args.output_dir, 'summary_strategy2.csv'), write_header=True) 
    ...
    bestidx = population_f.index(max(population_f))
    worstidx = population_f.index(min(population_f))
...
def find_max_index_after_removing_idx(lst, idx):
...
# ***************************************
    ...
    for epoch in range(2):
        ...
        for iter in range(max_iters):
            population, population_f, bestidx,worstidx, dist_matrics, paras, update_label = evolve(cost_func,epoch,iter, \
                                                                        max_iters,bounds,dim, \
                                                                        popsize,population,population_f, \
                                                                        paras)
            print('epoch:', epoch, 'iter:', iter, 'bestidx:', bestidx,'worstidx',worstidx)
...
# if iter == max_iters and epoch > 1000:
# # if iter == 1:
...
#         print('epoch:', epoch, 'iter:', iter, 'i_gen:', i_gen, 'success_num!!!!!', success_num)
