...
def update_summary(epoch, rowd_in, filename, write_header=True, log_wandb=False):
...
def update_summary2(epoch, train_metrics, eval_metrics, filename, write_header=False, log_wandb=False):
    rowd = OrderedDict(epoch=epoch)
...
#PSOMODIFIED: New logging helper for accuracy, precision, fitness
def log_results(epoch, acc, prec, fitness, logfile):
    with open(logfile, 'a') as f:
        f.write(f'Epoch: {epoch}, Accuracy: {acc:.4f}, Precision: {prec:.4f}, Fitness: {fitness:.4f}\n')
