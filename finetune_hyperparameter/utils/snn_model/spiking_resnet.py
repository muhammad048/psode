...
class SpikingResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 2) #TxNxC
        # return self.fc(x.mean(dim=0))
        x = self.fc(x) #TxNx100
        x = x.transpose(0,1) #NxTx100
        return x
...
def spiking_resnet34(pretrained=False, **kwargs):
    return _spiking_resnet(BasicBlock, [3, 4, 6, 3], **kwargs)

def spiking_resnet50(pretrained=False, **kwargs):
    return _spiking_resnet(Bottleneck, [3, 4, 6, 3], **kwargs)

def spiking_resnet101(pretrained=False, **kwargs):
    return _spiking_resnet(Bottleneck, [3, 4, 23, 3], **kwargs)

def spiking_resnet152(pretrained=False, **kwargs):
